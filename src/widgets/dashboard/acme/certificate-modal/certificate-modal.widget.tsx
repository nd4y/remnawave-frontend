...
export const AcmeCertificateModalWidget = (props: IProps) => {
    ...
    const form = useForm({
        ...
        validate: {
            credentialUuid: (value) => (value ? null : 'Pick a credential'),
            domains: (value) => (value.length === 0 ? 'At least one domain is required' : null),
            email: (value) => (/^[^@\s]+@[^@\s]+$/.test(value) ? null : 'Enter a valid e-mail'),
    ...
    const handleSubmit = form.onSubmit(async (values) => {
        const body = {
            ...
            nodes: values.nodeUuids.map((nodeUuid) => ({
                inboundTags:
                    certificate?.nodes.find((binding) => binding.nodeUuid === nodeUuid)
                        ?.inboundTags ?? [],
                nodeUuid
            })),
        ...
        if (isEdit) {
            await updateCertificate.mutateAsync({ variables: { ...body, uuid: certificate.uuid } })
        } else {
                ...
                <Stack gap="md">
                    ...
                    <Select
                        data={[
                            ...
                            { label: 'dns-persist-01', value: ACME_CHALLENGE_TYPE.DNS_PERSIST_01 }
                        ]}
                        label="Challenge"
                        {...form.getInputProps('challengeType')}
                    />

                    {isManual && !isDnsPersist && (
                    ...
                    {isDnsPersist && isProductionDirectory && (
                    ...
                    <Select
                        data={ACME_KEY_TYPES.map((keyType) => ({ label: keyType, value: keyType }))}
                        label="Key type"
                    ...
                    <Divider label="External account binding (optional)" labelPosition="left" />

                    <TextInput label="EAB key ID" {...form.getInputProps('eabKid')} />
                    <TextInput
                        description={isEdit ? 'Leave empty to keep the stored key' : undefined}
                        label="EAB HMAC key"
